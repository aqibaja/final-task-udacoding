...
class DiscountScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    _productBloc.add(InitialEventDiscount());
    return Scaffold(
        appBar: AppBar(
          elevation: 0.0,
          centerTitle: true,
        ...
        body: BlocBuilder<ProductBloc, ProductState>(
          builder: (context, state) {
            if (state is ProductDiscountInitial) {
              _productBloc.add(ViewProductDiscountEvent());
            }
            if (state is ProductDiscountLoading) {
              return LoadingProgress();
            }
            if (state is ProductDiscountLoaded) {
              return createListView(context, state.discountModel);
            }
            return LoadingProgress();
          },
        ));
...
Widget createListView(BuildContext context, List data) {
  List<DiscountModel> results = data;
